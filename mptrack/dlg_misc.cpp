...
BOOL CModTypeDlg::OnInitDialog()
{
	...
	m_TypeBox.SetItemData(m_TypeBox.AddString(_T("LushMPT LPTM")), MOD_TYPE_LSH);
	switch(m_nType)
	{
	...
	case MOD_TYPE_LSH:  m_TypeBox.SetCurSel(5); break;
...
void CModTypeDlg::UpdateDialog()
{
	...
	if(m_nType & (MOD_TYPE_MPT|MOD_TYPE_LSH) || (!(sndFile.GetType() & (MOD_TYPE_MPT|MOD_TYPE_LSH))  && sndFile.m_nTempoMode == TempoMode::Alternative))
		m_TempoModeBox.SetItemData(m_TempoModeBox.AddString(_T("Alternative")), static_cast<DWORD_PTR>(TempoMode::Alternative));
	if(m_nType & (MOD_TYPE_MPT|MOD_TYPE_LSH) || (!(sndFile.GetType() & (MOD_TYPE_MPT|MOD_TYPE_LSH)) && sndFile.m_nTempoMode == TempoMode::Modern))
	...
	if(m_nType & (MOD_TYPE_MPT|MOD_TYPE_LSH) || sndFile.GetMixLevels() == MixLevels::v1_17RC3) // In XM/IT, this is only shown for backwards compatibility with existing tunes
	...
	const bool XMorITorMPT = (m_nType & (MOD_TYPE_XM | MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_LSH));
	const bool isMPTM = (m_nType & (MOD_TYPE_MPT | MOD_TYPE_LSH));
	...
	if(m_nType & (MOD_TYPE_MPT | MOD_TYPE_IT | MOD_TYPE_XM | MOD_TYPE_LSH))
	{
