...
void CViewGlobals::UpdateView(UpdateHint hint, CObject *pObject)
{
	...
	CString s;
	if(sndFile.GetType() == MOD_TYPE_LSH)
		nTabCount = (sndFile.GetNumInstruments() + (CHANNELS_IN_TAB - 1)) / CHANNELS_IN_TAB;
	else
	...
	//Normally this is impossible with the channel mixer, but on new LPTM modules this is intended behavior (no instruments)
	if ((nTabIndex < 0) || (nTabIndex >= nTabCount)) {
		for (INSTRUMENTINDEX ichn = 0; ichn < CHANNELS_IN_TAB; ichn++) {
			SetDlgItemText(IDC_TEXT1 + ichn, _T(""));
			SetDlgItemText(IDC_EDIT9 + ichn, _T(""));
			m_channelColor[ichn].EnableWindow(FALSE);				
			s = _T("Record");
			SetDlgItemText(IDC_CHECK2 + ichn * 2, s);
	...
	{
		LockControls();
		//Use instrument mixer when handling LPTM files only.
		if(sndFile.GetType() == MOD_TYPE_LSH) {
			m_nActiveTab = static_cast<INSTRUMENTINDEX>(nTabIndex);
			for (INSTRUMENTINDEX ichn = 0; ichn < CHANNELS_IN_TAB; ichn++)
			{
				const INSTRUMENTINDEX nChn = m_nActiveTab * CHANNELS_IN_TAB + ichn;
				int numInst = sndFile.GetNumInstruments();
				const BOOL bEnable = (nChn < numInst) ? TRUE : FALSE;

				if(nChn < MAX_BASECHANNELS && bEnable)
				{
					const auto &chnSettings = sndFile.Instruments[nChn + 1];
					// Text
					if(bEnable)
						s = MPT_CFORMAT("Instrument {}")(nChn + 1);
					else
						s = _T("");
					SetDlgItemText(IDC_TEXT1 + ichn, s);
					// Channel color 
					m_channelColor[ichn].SetColor(nChn << 12 + 0xf0f0f0f0);
					m_channelColor[ichn].EnableWindow(bEnable);
					// Mute
					CheckDlgButton(IDC_CHECK1 + ichn * 2, chnSettings->dwFlags[INS_MUTE] ? TRUE : FALSE);
					// Surround
					s = _T("Record");
					SetDlgItemText(IDC_CHECK2 + ichn * 2, s);
					//CheckDlgButton(IDC_CHECK2 + ichn * 2, chnSettings.dwFlags[CHN_SURROUND] ? TRUE : FALSE);
					// Volume
					int vol = chnSettings->nGlobalVol;
					...
					// Channel name
					s = mpt::ToCString(sndFile.GetCharsetInternal(), chnSettings->GetName().c_str());
					SetDlgItemText(IDC_EDIT9 + ichn, s);
					((CEdit*)(GetDlgItem(IDC_EDIT9 + ichn)))->LimitText(MAX_INSTRUMENTNAME - 1);
				} else
				{
					SetDlgItemText(IDC_TEXT1 + ichn, _T(""));
					SetDlgItemText(IDC_EDIT9 + ichn, _T(""));
				...
				BOOL bIT = ((bEnable) && (sndFile.m_nType & (MOD_TYPE_IT|MOD_TYPE_MPT|MOD_TYPE_LSH)));
				...
				GetDlgItem(IDC_EDIT9 + ichn)->EnableWindow(((bEnable) && (sndFile.m_nType & (MOD_TYPE_XM|MOD_TYPE_IT|MOD_TYPE_MPT|MOD_TYPE_LSH))));	// channel name
				//Legacy plugin dialog disabled
				m_CbnEffects[ichn].EnableWindow(FALSE);
			}
		}
		else {
...
void CViewGlobals::OnMute(const CHANNELINDEX chnMod4, const UINT itemID)
{
	...
	if (pModDoc)
	{
		...
		if (pModDoc->GetModType() == MOD_TYPE_LSH)
			pModDoc->MuteInstrument(nChn + 1, b);
		else
...
void CViewGlobals::OnEditVol(const INSTRUMENTINDEX chnMod4, const UINT itemID)
{
	CModDoc* pModDoc = GetDocument();
	const CHANNELINDEX nChn = (CHANNELINDEX)(m_nActiveTab * CHANNELS_IN_TAB) + chnMod4;
	...
	{
		if (pModDoc->GetSoundFile().GetType() == MOD_TYPE_LSH) {
			PrepareUndo(chnMod4);
			pModDoc->GetSoundFile().Instruments[nChn + 1]->nGlobalVol = static_cast<uint16>(vol);
		...
		for (CHANNELINDEX iCh = 0; iCh < nLoopLimit; iCh++)
		{
			if(pScrollBar == (CScrollBar *) &m_sbVolume[iCh])
			{
				...
				if ((pos >= 0) && (pos <= 64))
				{
					...
					if(pModDoc->GetModType() == MOD_TYPE_LSH)
					{
						pModDoc->GetSoundFile().Instruments[nChn + 1]->nGlobalVol = static_cast<uint16>(pos);
...
void CViewGlobals::OnEditName(const CHANNELINDEX chnMod4, const UINT itemID)
{
	...
	if ((pModDoc) && (!m_nLockCount))
	{
		...
		if ((sndFile.GetType() & (MOD_TYPE_XM|MOD_TYPE_IT|MOD_TYPE_MPT)) && (nChn < sndFile.GetNumChannels()) && (s != sndFile.ChnSettings[nChn].szName))
		{
			...
			pModDoc->SetModified();
			pModDoc->UpdateAllViews(this, GeneralHint(nChn).Channels());
		} else if (sndFile.GetType() == MOD_TYPE_LSH) 
		{
			PrepareUndo(chnMod4);
			sndFile.Instruments[nChn + 1]->name = s;
			pModDoc->SetModified();
			pModDoc->UpdateAllViews(this, GeneralHint(nChn).Channels());
			pModDoc->UpdateAllViews(this, InstrumentHint(nChn + 1).Names());
		}
	}
}
