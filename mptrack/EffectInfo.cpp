...
static constexpr FlagSet<MODTYPE> MOD_TYPE_S3MITMPT = MOD_TYPE_S3M | MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_LSH;
static constexpr FlagSet<MODTYPE> MOD_TYPE_NOMOD = MOD_TYPE_S3M | MOD_TYPE_XM | MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_LSH;
static constexpr FlagSet<MODTYPE> MOD_TYPE_XMIT = MOD_TYPE_XM | MOD_TYPE_IT;
static constexpr FlagSet<MODTYPE> MOD_TYPE_XMITMPT = MOD_TYPE_XM | MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_LSH;
static constexpr FlagSet<MODTYPE> MOD_TYPE_ITMPT = MOD_TYPE_IT | MOD_TYPE_MPT  | MOD_TYPE_LSH;
static constexpr FlagSet<MODTYPE> MOD_TYPE_MPTLSH = MOD_TYPE_MPT  | MOD_TYPE_LSH;
...
static constexpr MPTEffectInfo gFXInfo[] =
{
	...
	{CMD_CHANNELVOLUME,	0,0,		0,	MOD_TYPE_S3MITMPT,	_T("Set Channel Volume / Set Instrument Volume")},
	...
	{CMD_DELAYCUT,		0x00,0x00,	0,	MOD_TYPE_MPT | MOD_TYPE_LSH,	_T("Note Delay and Cut")},
	...
	{CMD_FINETUNE,			0,0,	0,	MOD_TYPE_MPT | MOD_TYPE_LSH,	_T("Finetune")},
	{CMD_FINETUNE_SMOOTH,	0,0,	0,	MOD_TYPE_MPT | MOD_TYPE_LSH,	_T("Finetune (Smooth)")},
...
bool EffectInfo::GetEffectInfo(UINT ndx, CString *s, bool bXX, ModCommand::PARAM *prangeMin, ModCommand::PARAM *prangeMax) const
{
	...
	if ((prangeMin) && (prangeMax))
	{
		...
		case CMD_GLOBALVOLUME:
			nmax = (sndFile.GetType() & (MOD_TYPE_LSH | MOD_TYPE_IT | MOD_TYPE_MPT)) ? 128 : 64;
			break;
	...
	case CMD_GLOBALVOLUME:
		{
			...
			if((sndFile.GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_S3M | MOD_TYPE_LSH)) && param > maxVal)
				s = _T("undefined");
			else
		...
		if (gFXInfo[ndx].paramMask == 0xF0)
		{
				...
				&& ((gFXInfo[ndx].paramValue & 0xF0) == 0x90) && ((param & 0xF0) == 0x90))
			{
					...
					&& ((gFXInfo[ndx].paramValue & 0xF0) == 0x70) && ((param & 0xF0) == 0x70))
				{
					...
					case 0x0D:	if(sndFile.GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH)) { s = _T("7D: Force Pitch Env"); break; }
								[[fallthrough]];
					case 0x0E:	if(sndFile.GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH)) { s = _T("7E: Force Filter Env"); break; }
								[[fallthrough]];
				...
				} else
				{
					...
					if(gFXInfo[ndx].effect == CMD_S3MCMDEX)
					{
						...
						case 0xD0: // note delay
							...
							if(((param & 0x0F) == 1) || ((param & 0x0F) == 0 && (sndFile.GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_LSH))))
								s = _T("1 tick");
							else
								s += _T(" ticks");
							break;
...
static constexpr MPTVolCmdInfo gVolCmdInfo[] =
{
	...
	{VOLCMD_OFFSET,			MOD_TYPE_MPTLSH,	_T("Sample Cue")},
};
