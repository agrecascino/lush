...
void CViewInstrument::UpdateNcButtonState()
{
	...
	for (UINT i=0; i<ENV_LEFTBAR_BUTTONS; i++) if (cLeftBarButtons[i] != ID_SEPARATOR)
	{
		...
		case ID_ENVSEL_PITCH:		if (!(sndFile.GetType() & (MOD_TYPE_IT|MOD_TYPE_MPT|MOD_TYPE_LSH))) dwStyle |= NCBTNS_DISABLED;
		...
		case ID_ENVELOPE_CARRY:		if (!(sndFile.GetType() & (MOD_TYPE_IT|MOD_TYPE_MPT|MOD_TYPE_LSH))) dwStyle |= NCBTNS_DISABLED;
		...
		case ID_ENVELOPE_PITCH:		if (!(sndFile.GetType() & (MOD_TYPE_IT|MOD_TYPE_MPT|MOD_TYPE_LSH))) dwStyle |= NCBTNS_DISABLED; else
									if (EnvGetPitchEnv()) dwStyle |= NCBTNS_CHECKED; break;
		case ID_ENVELOPE_FILTER:	if (!(sndFile.GetType() & (MOD_TYPE_IT|MOD_TYPE_MPT|MOD_TYPE_LSH))) dwStyle |= NCBTNS_DISABLED; else
...
void CViewInstrument::OnRButtonDown(UINT flags, CPoint pt)
{
	...
	if((pModDoc) && (Menu.LoadMenu(IDR_ENVELOPES)))
	{
		...
		if(pSubMenu != nullptr)
		{
			...
			pSubMenu->EnableMenuItem(ID_ENVELOPE_CARRY, (sndFile.GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_LSH)) ? MF_ENABLED : MF_GRAYED);
