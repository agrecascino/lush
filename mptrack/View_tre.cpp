	...
	if(info.hSamples && sampleHint.GetType()[HINT_MODTYPE | HINT_SMPNAMES | HINT_SAMPLEINFO | HINT_SAMPLEDATA])
	{
		...
		for(SAMPLEINDEX nSmp = smin; nSmp <= smax; nSmp++)
		{
			...
			if(nSmp <= sndFile.GetNumSamples())
			{
				...
				if(sndFile.GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH))
				{
			...
			case MODITEM_SAMPLE:
				{
					...
					if(sndFile != nullptr)
					{
						...
						const bool menuForThisSample = (sample.HasSampleData() && sndFile->GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH)) || hasPath;
						...
						{
							AppendMenu(hMenu, MF_SEPARATOR, NULL, _T(""));
							if(menuForThisSample) AppendMenu(hMenu, MF_STRING | ((sndFile->GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH) || hasPath) ? 0 : MF_GRAYED), ID_MODTREE_SETPATH, _T("Set P&ath"));
