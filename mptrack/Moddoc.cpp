...
const TCHAR FileFilterLSH[] = _T("LushMPT Modules (*.lptm)|*.lptm||");
...
const CString ModTypeToFilter(const CSoundFile& sndFile)
{
	...
	switch(modtype)
	{
		...
		case MOD_TYPE_LSH: return FileFilterLSH;
...
bool CModDoc::OnSaveDocument(const mpt::PathString &filename, const bool setPath)
{
	...
	try
	{
		...
		if(f)
		{
			...
			case MOD_TYPE_LSH: ok = m_SndFile.SaveIT(f, filename); break;
...
{
	if(m_SndFile.GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH) && !SaveAllSamples())
		return FALSE;
...
void CModDoc::InitializeMod()
{
	...
	if (!m_SndFile.m_nChannels)
	{
		...
		if(GetModType() & (MOD_TYPE_MPT | MOD_TYPE_LSH))
		{
	...
	if (!m_SndFile.m_nSamples)
	{
		...
		if (m_SndFile.GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_XM | MOD_TYPE_LSH))
		{
...
{
	return (m_SndFile.GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_S3M | MOD_TYPE_LSH)) && TrackerSettings::Instance().MiscSaveChannelMuteStatus;
}
...
bool CModDoc::SurroundChannel(CHANNELINDEX nChn, bool surround)
{
	...
	if(!(m_SndFile.GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_LSH))) surround = false;

	if(surround != m_SndFile.ChnSettings[nChn].dwFlags[CHN_SURROUND])
	{
		// Update channel configuration
		if(m_SndFile.GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_LSH)) SetModified();
...
bool CModDoc::SetChannelDefaultPan(CHANNELINDEX nChn, uint16 nPan)
{
	...
	if(m_SndFile.ChnSettings[nChn].nPan != nPan || m_SndFile.ChnSettings[nChn].dwFlags[CHN_SURROUND])
	{
		...
		if(m_SndFile.GetType() & (MOD_TYPE_S3M | MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_LSH)) SetModified();
		ok = true;
	}
...
{
	if (p)
		p->Enable((m_SndFile.GetType() & (MOD_TYPE_XM | MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_LSH)) ? TRUE : FALSE);
}
...
{
	if (p)
		p->Enable(((m_SndFile.GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_LSH)) || !m_SndFile.GetFileHistory().empty()) ? TRUE : FALSE);
}
...
void CModDoc::OnViewTempoSwingSettings()
{
	...
	} else if(GetModType() & (MOD_TYPE_MPT | MOD_TYPE_LSH))
	{
