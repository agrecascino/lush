	...
	if ((pszText) && (uId))
	{
		...
		case IDC_EDIT14:
			...
			} else if(m_nSample)
			{
				...
				} else if(m_sndFile.GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_LSH))
				{
					if(val > 0)
...
void CCtrlSamples::UpdateView(UpdateHint hint, CObject *pObj)
{
	...
	if(hintType[HINT_MODTYPE])
	{
		...
		// AutoVibrato
		b = (m_sndFile.GetType() & (MOD_TYPE_XM|MOD_TYPE_IT|MOD_TYPE_MPT|MOD_TYPE_LSH)) ? TRUE : FALSE;
		...
		// Global Volume or Mixer Volume on LPTM
		b = (m_sndFile.GetType() & (MOD_TYPE_IT|MOD_TYPE_MPT|MOD_TYPE_LSH)) ? TRUE : FALSE;
	...
	if (hintType[HINT_MODTYPE | HINT_SAMPLEINFO])
	{
		...
		const bool hasSustainLoop = !isOPL && ((m_sndFile.GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_LSH)) || (m_nSample <= m_sndFile.GetNumSamples() && m_sndFile.GetSample(m_nSample).uFlags[CHN_SUSTAINLOOP]));
	...
	if(hintType[HINT_MODTYPE | HINT_SAMPLEINFO | HINT_SMPNAMES])
	{
		...
		GetDlgItem(IDC_CHECK2)->EnableWindow((m_sndFile.SampleHasPath(m_nSample) && m_sndFile.GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH)) ? TRUE : FALSE);
	}
