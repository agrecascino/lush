...
void CNoteMapWnd::EnterNote(UINT note)
{
	...
	{
		if (!m_bIns && (sndFile.GetType() & (MOD_TYPE_IT|MOD_TYPE_MPT|MOD_TYPE_LSH)))
		{
...
bool CNoteMapWnd::HandleChar(WPARAM c)
{
	...
	if ((pIns) && (m_nNote < NOTE_MAX))
	{
		...
		else if ((!m_bIns) && (sndFile.m_nType & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_LSH)))	//in note column
		{
...
void CCtrlInstruments::UpdateView(UpdateHint hint, CObject *pObj)
{
	...
	if(hintType[HINT_MODTYPE])
	{
		...
		const BOOL bITandMPT = ((m_sndFile.GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_LSH)) && (m_sndFile.GetNumInstruments())) ? TRUE : FALSE;
		const BOOL bITandXM = ((m_sndFile.GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_XM | MOD_TYPE_LSH))  && (m_sndFile.GetNumInstruments())) ? TRUE : FALSE;
		const BOOL bMPTOnly = ((m_sndFile.GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH)) && (m_sndFile.GetNumInstruments())) ? TRUE : FALSE;
	...
	if(hintType[HINT_MODTYPE | HINT_INSTRUMENT])
	{
		...
		m_SliderCutSwing.EnableWindow(pIns != nullptr && (m_sndFile.GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH) || pIns->nCutSwing != 0));
		m_SliderResSwing.EnableWindow(pIns != nullptr && (m_sndFile.GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH) || pIns->nResSwing != 0));
		m_CbnFilterMode.EnableWindow (pIns != nullptr && (m_sndFile.GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH) || pIns->filterMode != FilterMode::Unchanged));
		m_CbnResampling.EnableWindow (pIns != nullptr && (m_sndFile.GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH) || pIns->resampling != SRCMODE_DEFAULT));
		m_SliderAttack.EnableWindow  (pIns != nullptr && (m_sndFile.GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH) || pIns->nVolRampUp));
		::EnableWindow(::GetDlgItem(m_hWnd, IDC_EDIT2), pIns != nullptr && (m_sndFile.GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH) || pIns->nVolRampUp));
		...
		if (pIns)
		{
			...
			// Filter
			if (m_sndFile.GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_LSH))
			{
			...
			m_CheckPitchTempoLock.EnableWindow((m_sndFile.GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH) || pIns->pitchToTempoLock.GetRaw() > 0) ? TRUE : FALSE);
			...
			if(m_sndFile.GetType() & (MOD_TYPE_XM|MOD_TYPE_IT|MOD_TYPE_MPT|MOD_TYPE_LSH))
			{
...
void CCtrlInstruments::OnSetPanningChanged()
{
	...
	if ((!IsLocked()) && (pIns))
	{
		...
		if(b && m_sndFile.GetType() & (MOD_TYPE_IT|MOD_TYPE_MPT|MOD_TYPE_LSH))
		{
