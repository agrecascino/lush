...
void ModCommand::Convert(MODTYPE fromType, MODTYPE toType, const CSoundFile &sndFile)
{
	...
	const bool oldTypeIsMOD = (fromType == MOD_TYPE_MOD), oldTypeIsXM = (fromType == MOD_TYPE_XM),
		...
		oldTypeIsMPT = (fromType & MOD_TYPE_MPT | MOD_TYPE_LSH), oldTypeIsMOD_XM = (oldTypeIsMOD || oldTypeIsXM),
	...
	const bool newTypeIsMOD = (toType == MOD_TYPE_MOD), newTypeIsXM =  (toType == MOD_TYPE_XM),
		...
		newTypeIsMPT = (toType & MOD_TYPE_MPT | MOD_TYPE_LSH), newTypeIsMOD_XM = (newTypeIsMOD || newTypeIsXM),
	...
	if(command == CMD_PANNING8)
	{
		...
		} else if(oldTypeIsS3M)
		{
			...
			{
				// surround remap
				command = (toType & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_LSH)) ? CMD_S3MCMDEX : CMD_XFINEPORTAUPDOWN;
				param = 0x91;
			} else
			{
