...
PlayBehaviourSet CSoundFile::GetSupportedPlaybackBehaviour(MODTYPE type)
{
	...
	case MOD_TYPE_IT:
		...
		if(type & (MOD_TYPE_MPT | MOD_TYPE_LSH))
		{
...
mpt::ustring CSoundFile::GetNoteName(const ModCommand::NOTE note, const INSTRUMENTINDEX inst) const
{
	...
	if(ModCommand::IsNote(note) && GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH) && inst >= 1 && inst <= GetNumInstruments() && Instruments[inst] && Instruments[inst]->pTuning)
	{
...
void CSoundFile::ChangeModTypeTo(const MODTYPE newType, bool adjust)
{
	...
	if(oldType == MOD_TYPE_S3M && newType & (MOD_TYPE_MPT | MOD_TYPE_LSH) && m_opl)
