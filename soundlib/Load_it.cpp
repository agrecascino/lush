...
const uint16 verLshFileMinValueLoad = 0xE00; // If cwtv-field is at least this value,
											  // this is an LPTM file and will be loaded as such.
const uint16 verLshFileVer = 0xE00; 
...
bool CSoundFile::ReadIT(FileReader &file, ModLoadingFlags loadFlags)
{
	...
	} else
	{
		if(fileHeader.cwtv > 0x888 && fileHeader.cwtv <= 0xFFF)
		{
			...
			if(mptStartPos >= 0x100 && mptStartPos < file.GetLength())
			{
				...
				{
					if (fileHeader.cwtv >= verLshFileMinValueLoad)
					{
						SetType(MOD_TYPE_LSH);
					} else if(fileHeader.cwtv >= verMptFileVerLoadLimit)
					{
	...
	for(PATTERNINDEX pat = 0; pat < numPats; pat++)
	{
		...
		while(row < numRows && patternData.CanRead(1))
		{
			...
			if(chnMask[ch] & 1)	// Note
			{
				...
				if(note < 0x80)
					note += NOTE_MIN;
				if(!(GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH)))
				{
	...
	m_modFormat.formatName = (GetType() == MOD_TYPE_LSH) ? U_("LushMPT LPTM") : m_modFormat.formatName;
	m_modFormat.type = (GetType() == MOD_TYPE_MPT) ? U_("mptm") : U_("it");
	m_modFormat.type = (GetType() == MOD_TYPE_LSH) ? U_("lptm") : m_modFormat.type;
...
bool CSoundFile::SaveIT(std::ostream &f, const mpt::PathString &filename, bool compatibilityExport)
{
	...
	const CModSpecifications &specs = (GetType() == MOD_TYPE_MPT ? ModSpecs::mptm : 
		(GetType() == MOD_TYPE_LSH ? ModSpecs::lptm : (compatibilityExport ? ModSpecs::it : ModSpecs::itEx)));
	...
	if(GetType() == MOD_TYPE_MPT)
	{
		...
		itHeader.cmwt = 0x888;
	} else if(GetType() == MOD_TYPE_LSH)
	{
		itHeader.cwtv = verLshFileVer;
		itHeader.cmwt = 0x888;
	} else
	{
		// IT
	...
	for(PATTERNINDEX pat = 0; pat < itHeader.patnum; pat++)
	{
		...
		for(ROWINDEX row = 0; row < writeRows; row++)
		{
			...
			for(CHANNELINDEX ch = 0; ch < maxChannels; ch++, m++)
			{
				...
				if (note == NOTE_FADE && !(GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH))) note = 0xF6;
				if (m->instr) b |= 2;
	...
	for(SAMPLEINDEX smp = 1; smp <= itHeader.smpnum; smp++)
	{
		...
		itss.ConvertToIT(sample, GetType(), compress, itHeader.cmwt >= 0x215, GetType() & (MOD_TYPE_MPT | MOD_TYPE_LSH));
...
#define WRITEMODULAR(code, field) \
	...
	if((GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_LSH)) && GetNumChannels() > 64)
	{
